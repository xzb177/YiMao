...
contains docker-compose.yml 'PUID:[[:space:]]*\$\{PUID:-[1-9][0-9]*\}'
contains docker-compose.yml 'PGID:[[:space:]]*\$\{PGID:-[1-9][0-9]*\}'
contains .env.example '^PUID=[1-9][0-9]*$'
contains .env.example '^PGID=[1-9][0-9]*$'
contains docker-entrypoint.sh 'PUID=\$\{PUID:-[1-9][0-9]*\}'
contains docker-entrypoint.sh 'PGID=\$\{PGID:-[1-9][0-9]*\}'
contains docker-entrypoint.sh '^exec su-exec yimao "\$@"$'
contains docker-compose.resetpw.yml 'PUID:[[:space:]]*"0"'
contains docker-compose.resetpw.yml 'PGID:[[:space:]]*"0"'
contains docker-compose.resetpw.yml '/var/run/docker.sock:/var/run/docker.sock'
...
contains scripts/ops.sh 'docker build --target verify.*>&2'
contains scripts/ops.sh 'docker build --build-arg.*>&2'
