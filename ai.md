...
- **消息内容**: 提示用户重新绑定账户
- **用户列表**:
  - 1448188137, 1500067755, 2057707130, 2077655039, 2112026903
  - 5278360890, 5751551914, 5779291957, 5953974304, 6180672165
  - 6606122119, 6702279264, 6731537514, 6998014448, 7443038555
  - 7564102861, 7569472328, 8025047064, 8287241362, 8379211153, 915561290
- **代码变更**: 添加 `GetAllTelegramUsers()` 方法到 UserMappingService
- **状态**: ✅ 已发送
...
### fix: 我的请求状态 - 改进状态显示逻辑 ✅
- **时间**: 2026-03-20 23:02
- **问题**: "我的请求"列表中所有订阅都显示"进行中"，用户体验不佳
- **根本原因**:
  1. MoviePilot 订阅系统只有 P/R/S 三种状态，没有"完成"状态
  2. 之前的 Emby 检查逻辑不可靠（媒体没有关联 TMDB ID）
  3. 用户无法区分哪些正在下载、哪些在搜索、哪些可能失败
- **优化内容**:
  1. **电视剧状态优化**:
     - `lack_episode == 0 && total_episode > 0` → ✅ 已完成
     - `lack_episode < total_episode` → ⬇️ 下载中（显示进度）
     - 否则保持原状态（R=订阅中，S=搜索中）
  2. **电影状态优化**:
     - 在 Emby 中找到 → ✅ 已完成
     - 订阅超过 30 天且不在 Emby → ❌ 失败
     - 否则显示原状态
  3. **改进 Emby 搜索**：
     - 使用 SearchHints 端点
     - 支持年份匹配
     - 中文标题智能匹配
- **修改文件**:
  - `internal/services/moviepilot.go` - 状态判断逻辑重构
- **Docker**: ✅ 已推送 xzb177/yimao:latest
- **Digest**: `sha256:6c5c9c692e6d572745ef8b87ee87e8fabc73a6d4a15c74d534484f11d2ffc359`
- **状态**: ✅ 已部署
...
- **根本原因**:
  1. MoviePilot 没有 "C"（已完成）状态
  2. 订阅完成后仍保持 "R" 状态，需通过 `lack_episode` 字段判断
  3. 订阅删除后本地仍保存旧状态
- **修复内容**:
  1. 添加 `LackEpisode` 字段到 `SubscribeStatus` 结构
  2. 当 `lack_episode = 0` 时，标记为 "C"（已完成）
  3. 订阅不存在时标记为 "X"（已取消）
  4. 优化文案："🔄 重新搜索" → "🔄 订阅中"
- **修改文件**:
  - `internal/services/moviepilot.go` - 添加 LackEpisode 字段
  - `internal/services/review.go` - 状态判断逻辑优化
- **状态**: ✅ 已部署

---

### fix: 订阅状态不更新 - 修复过期订阅处理 ✅
- **时间**: 2026-03-20
- **问题**: "我的求片"中订阅状态永远显示"重新搜索"等中间状态，不更新
- **根本原因**:
  - MoviePilot 中的订阅被删除后（如已完成、取消）
  - 本地 review_requests.json 仍保存旧的 subscription_state="R"
  - 状态同步代码未处理"订阅不存在"的情况
- **修复内容**:
  - 当订阅ID不在 MoviePilot 返回列表时，标记为 "X"（已取消）
  - 界面显示"🚫 已取消"而不是一直"🔄 重新搜索"
- **修改文件**:
  - `internal/services/review.go` - updateAllSubscriptionStatus 函数
- **状态**: ✅ 已部署
...
- **提交**: 81248ed, 07718a2
- **操作**:
  - Git: ✅ 推送到 origin/master (080e4a8..81248ed)
  - Docker: ✅ 构建并推送到 xzb177/yimao:latest
  - 容器: ✅ 已重启（健康状态 running）
  - 镜像大小: 45.4MB
- **状态**: ✅ 全部完成
...
#### 状态**: ✅ 全部完成并部署
- **Git 提交**: 07718a2 (优化), 81248ed (Go 1.24 版本修复)
- **Docker**: ✅ 构建并推送到 xzb177/yimao:latest
- **Digest**: `sha256:9bbe0096a50269c8e8496ace2c71f13d1a5fc6872452ee61f3d6d75ea28010d7`
- **镜像大小**: 45.4MB
- **容器**: ✅ 已重启 (健康状态 running)
...
- **完成内容**:
  1. **Docker 镜像构建**
     - 镜像名: `xzb177/yimao:latest`
     - 构建状态: ✅ 成功
     - 镜像大小: 14.3MB (压缩后)
     - 磁盘占用: 44.6MB
     - 推送状态: ✅ 已推送到 Docker Hub
     - Digest: `sha256:47d909cdedceedb76bd793dd0921f7ebe5ec8f116bee609d152b63b57cccc68e`
  2. **README.md 全面优化**
     - 添加居中布局和徽章
     - 优化功能介绍，分类更清晰（主流程/增强/管理）
     - 新增「候选列表」功能说明
     - 添加三种部署方式说明（Docker/脚本/源码）
     - 改进命令说明表格
     - 添加架构图和代码结构说明
     - 优化底部导航和页脚
- **修改文件**:
  - `README.md` - 完全重写
- **Docker Hub**: https://hub.docker.com/r/xzb177/yimao
- **状态**: ✅ 已完成并推送
...
- **删除数量**: 17 条无效反馈 (ID: 4, 9-24)
- **删除原因**:
  - 空描述测试数据 (12 条)
  - 测试内容 ("测试一下", "看看")
  - 误操作 ("/start")
- **保留数量**: 7 条有效反馈 (ID: 1, 2, 3, 5, 6, 7, 8)
- **保留原因**:
  - 有实质描述内容
  - 状态为 open/processing/reply (未解决)
  - 有管理员回复交互
- **状态**: ✅ 已完成
...
- **范围**: 6 个文件变更，+880/-13 行
- **新增文件**:
  - `ai/agent.go` - AI Agent 编排层
  - `ai/client.go` - Claude/Zhipu API 客户端
- **功能**: 完整集成 AI 心情推荐系统
- **状态**: ✅ 已推送到 origin/master
...
- **问题emoji**:
  - `👮‍♂️` (警👮 + ZWJ + 男性符号♂️)
  - `👨‍💼` (人👨 + ZWJ + 公文包💼)
- **解决方案**: 将组合emoji替换为单一emoji `🛡️`
- **修改位置**:
  - `internal/handlers/admin.go` - 3处
  - `internal/api/router.go` - 1处
- **效果**: 消除乱码，显示更清晰美观

## 2026-03-05

### feat: 添加一键更新脚本 ✅
- **功能**: 新用户可使用 `./update.sh` 快速更新部署
- **脚本功能**:
  - 自动备份未提交的更改
  - 拉取最新代码
  - 重新构建 Docker 镜像
...
### docs: 更新 README.md 添加一键更新脚本用法 ✅
- **修改**: 在"常用操作"部分添加 `./update.sh` 一键更新命令
- **位置**: README.md 第 91-102 行
- **内容**: 将一键更新脚本列为推荐的更新方式
- **效果**: 用户可以直接看到最便捷的更新方法
